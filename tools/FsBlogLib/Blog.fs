...
module Blog =
  let emptyModel =
    { Root = ""; SiteTitle =""; SiteSubtitle = ""; MonthlyPosts = [||]; Posts = [||]; TaglyPosts = [||]; GenerateAll = false; IsPostCategoryPage = false; Category = { Name = "" } }
  ...
  let GeneratePostCategoryPage layouts template blogIndex model posts urlFunc needsUpdate infoFunc getPosts getCategory =
    for item in posts do
      let category = getCategory item
      let model = { model with GenerateAll = true; Posts = Array.ofSeq (getPosts item); IsPostCategoryPage = true; Category = { Name = category } }
      let razor = FsBlogLib.Razor(layouts, Model = model)
      let target = urlFunc item
      EnsureDirectory(Path.GetDirectoryName(target))
      if not (File.Exists(target)) || needsUpdate item then
