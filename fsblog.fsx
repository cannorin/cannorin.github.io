...
let buildSite routing updateTagArchive =
    ...
    let noModel = { Blog.emptyModel with Root = root; SiteTitle = title; SiteSubtitle = subtitle; GenerateAll = true }
    ...
    Blog.GeneratePostCategoryPage
        ...
        (fun (_, _, p) -> p)
        (fun (y, m, _) -> sprintf "Archive: %d %s" y m)
